...
## Composition
Everything ships as packages and are composable at build time. 
...
### Adapters
Adapters provide infrastructure for the core service - e.g. authentication, storage, emails, workflow engines. These 
are all required by the core, but are swoppable so that anyone can use their own implemnenations.

By default Headless LMS ships with sane defaults for self-hosting so everything works out of the box.

### Plugins
Plugins provide additional functionality to the core service - e.g. analytics, notifications like slack, CRM integrations etc.

Plugins define a set of actions and triggers that are used by the Automation Engine to extend the core functionality. Things 
like "Send a Slack message when someone subscribes".
...
- The core is split into **domains** of **bounded contexts**. Each is self-contained and exposes one public surface.
...
- **identity** — the person and authentication only. Owns the global `users` record other contexts reference by id; mirrors Better Auth (the credentials/session system of record) via hooks, for every account including learners. No org participation, no roles.
- **organizations** — the tenant root every org-scoped context FKs to. Owns Organization, OrgUser (a person's participation under one role — staff and learners alike, and the target of every actor-shaped FK), Invitation, the role/permission matrix (`owner | admin | instructor | student`; instructor course-scoped), and the member-management operations (invite / change-role / remove / list). Better Auth's organization plugin is the source of truth, mirrored read-only via `organizationHooks`; writes go through Better Auth via the `OrgAdmin` port.
