...
const containerConfig: Config = {
  ...
  mcpSelectOrgPage: 'http://localhost:8001/oauth/select-org',
  credentialStoreKey: '',
...
describe('OAuth token endpoint — form-encoded body', () => {
  ...
  it('POST /api/auth/oauth2/token with a form body reaches the handler (not 415/404)', async () => {
    // Regression for Critical 1: Fastify must not reject form-encoded bodies
    // before they reach Better Auth. The oauth provider mounts the token
    // endpoint at /oauth2/token. With an invalid code/client the request must
    // fail with an OAuth error (400/401) — a 415 means the form parser is
    ...
    //   - NOT 404  → it routed to the real /oauth2/token handler
