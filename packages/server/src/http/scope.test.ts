...
function container(opts: {
  ...
  user?: { id: string } | null;
  orgUser?: { id: string; orgId: string; role: string } | null;
}): Container {
  return {
    organizations: {
      getByExternalId: async () => opts.org ?? null,
      getOrgUserByUser: async () => opts.orgUser ?? null,
    },
...
const orgUser = { id: 'mem_1', orgId: 'org_1', role: 'owner' };

describe('resolveScope', () => {
  it('resolves { orgId, userId, authOrgId } for a staff member', async () => {
    const scope = await resolveScope(
      container({ org: acme, user: { id: 'usr_1' }, orgUser }),
      req({ id: 'ext_1' }),
    );
    expect(scope).toEqual({ orgId: 'org_1', userId: 'usr_1', authOrgId: 'ext_org_1' });
  });

  it('throws NoActiveOrgError when the session user has no org orgUser', async () => {
    await expect(
      resolveScope(container({ org: acme, user: { id: 'usr_1' }, orgUser: null }), req({ id: 'ext_1' })),
  ...
  it('throws NoActiveOrgError when the session carries no active org', async () => {
    await expect(
      resolveScope(container({ org: acme, user: { id: 'usr_1' }, orgUser }), req({ id: 'ext_1' }, null)),
    ).rejects.toBeInstanceOf(NoActiveOrgError);
  });

  it('throws NoActiveOrgError when the active org is not found', async () => {
    await expect(
      resolveScope(container({ org: null, user: { id: 'usr_1' }, orgUser }), req({ id: 'ext_1' })),
    ).rejects.toBeInstanceOf(NoActiveOrgError);
  });

  it('throws NoActiveOrgError when there is no domain user for the session user', async () => {
    await expect(
      resolveScope(container({ org: acme, user: null, orgUser }), req({ id: 'ext_1' })),
    ).rejects.toBeInstanceOf(NoActiveOrgError);
  });
});
