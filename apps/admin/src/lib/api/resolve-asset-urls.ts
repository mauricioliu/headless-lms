...
/**
 * Render-time asset URL signing for editor content.
 *
 * Media nodes persist a stable `assetId` alongside a `url` that was presigned
 * at upload time and has long since expired. Before content is rendered (the
 * editor's initial value, the preview, any student-facing surface), the
 * server walks the config and swaps every referenced URL for a freshly
 * presigned one via the existing download-url brokering — so access stays
