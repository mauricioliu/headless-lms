...
/**
 * Client island for `/onboarding`, where the `(dashboard)` layout redirects a
 * session resolved as `no-organization` (signed in, not a member of any org).
 * Creates the org — which the API also makes the session's active org — then
 * hard-navigates to the dashboard so the server session resolver re-runs and
 * picks up the new active org.
 */
export function OnboardingView() {
  ...
  async function onSubmit(values: Values) {
    ...
    // load of the dashboard lets the server session resolver re-run.
