...
Implement the `Integration` contract from `@headless-lms/core/types` (the zod helpers in `@headless-lms/utils` cover input validation), drop the folder into your installation's plugins directory, and restart. Integration packages depend only on `@headless-lms/core/types` and `@headless-lms/utils` — never on the server.
