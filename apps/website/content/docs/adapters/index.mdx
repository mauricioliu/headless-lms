...
Because each slot is just an interface, you can replace any adapter with your own implementation — implement the port from `@headless-lms/core/types` and pass it in the same way. Installations scaffolded with `npm create headless-lms` come wired with the default adapters shown above.
